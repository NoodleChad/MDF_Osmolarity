...
# In toronto_set_up, change line 39 and 40 so that the min is 8.1e-6 and the max is 8.1e-1
...
optmdfpathway_base_variables["ATPM"].bounds(62.5,1000) # For NaCl
#optmdfpathway_base_variables["ATPM"].bounds(80,1000) # For KCl
initial_k = .857
current_k = initial_k
while current_k >= 0.01:
    ...
    if growth_optimization_result["status"] == "Optimal":
        ...
        current_k = current_k - 0.01
    else:
        print("Infeasible!")
        break
with open("./toronto_results/analysis_ii/analysis_II_nacl.csv", "w") as f:
