...
def add_osmolarity_constraints(optmdfpathway_base_problem: pulp.LpProblem,
    ...
    charge_expression = pulp.LpAffineExpression(name="charge_sum")
    with open('iML1515_metabolites.pickle', "rb") as f:
        model_pK = pickle.load(f)
    new_ID = [item + "_c" for item in list(model_pK.ID)]
    for variable in optmdfpathway_base_problem.variables():
        ...
        if metabolite_id in metabolite_weights.keys():
            ...
            weight = 1.0
        if metabolite_id in new_ID:
            weight_charge = model_pK.C[new_ID.index(metabolite_id)]
        else:
            weight_charge = 0
        ...
        # Add charge expression
        charge_expression += weight_charge * y_variable

    #potassium will be use as a simple cation both in charge problem and osmolarity
    k_conc = pulp.LpVariable(
        name="k_conc",
        lowBound=0
    )
    sum_expression += 1.0 * k_conc
    charge_expression += 1.0 * k_conc
    ...
    Charge_sum_constraint = charge_expression == 0
    optmdfpathway_base_problem += Charge_sum_constraint
